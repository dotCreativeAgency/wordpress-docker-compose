<?php
/*
Plugin Name: Display Categories Widget2
...
License: GPL
*/
class DisplayCategoriesWidget2 extends WP_Widget

{
    function __construct()
    {
        $widget_ops = array(
            'classname' => 'DisplayCategoriesWidget2',
            'description' => 'Displays categories'
        );
        parent::__construct('DisplayCategoriesWidget2', 'Display Categories Widget2', $widget_ops);
    }
...
?>
    ...
    function widget($args, $instance)
    {
        ...
        if ($instance['show_format'] == 0) {
            echo "<style>.dcw_c1 {float:left; width:100%} .dcw_c2 {float:left; width:50%} .dcw_c3 {float:left; width:33%} .dcw ul.children li{ display: inline; padding: 0 4px;font-style: italic;font-size: .85em; border-right: 1px solid #1e73be;} .dcw ul.children li:first-child {border-left: 1px solid #1e73be; } .dcw {margin-left: 5px;}.dcw li{margin-top: .5em;font-size: 0.9em;padding:0 5px!important;color: #2a5ad3;}.dcw-cat{padding-bottom:5px;}  </style>";
            
				if ( is_category() ) {
	            	$current_cat = get_query_var('cat');
						$cat_id = $current_cat;
				}
				echo '<h4 class="dcw-cat">'.get_cat_name( $cat_id ).'</h4>';
				echo "<ul class='dcw'>";
        ...
        if ($instance['show_format'] == 2) {
            $dcw_disp_widget_id = dcw2_get_widget_id($this->id);
?>
    <form action="<?php
...
function dcw2_get_widget_id($dcw_wid)
{
    $dcw_process_id = explode('-', $dcw_wid);
    return $dcw_process_id[1];
}

add_action('widgets_init', function() {return register_widget("DisplayCategoriesWidget2");}); ?>